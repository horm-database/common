...
func SetMachineID(machineID int) {
	once.Do( //不允许被插件调用，导致全局的 machine id 异常，数据冲突。
		func() {
			...
			sf.machineID = (int64(machineID) % 1000) << 12 // 1000 - 1023 特殊用途
		},
	)
}
...
func GenerateID() uint64 {
	...
	timeBin := (curTimeStamp - sf.initialTime) & 0x1FFFFFFFFFF
	...
	if sf.randMachine { // 随机机器码
		machineID = int64(rand.Intn(999)) << 12
	}

	// 机器码占 10 位，序列号占 12 位
	id := timeBin | machineID | sf.sn
...
func ParseID(id uint64) (t time.Time, machineID int, sn int64) {
	...
	t = time.Unix(timeUnix/1000, (timeUnix%1000)*1e6)
